...
const WS_URL = 'ws://94.104.203.203:4000';
...
export default function ClientSpaceComponent() {
  ...
  const logMessage = (msg: string) => setLogs((prev) => [...prev, msg]);
  ...
  const renderStars = (rating: number | null) => {
    return Array(5)
      ...
      .map((_, i) => (
        <Star
          key={i}
          className={`h-4 w-4 ${i < (rating ?? 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
        />
      ));
  };
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 sm:gap-6">
        <Card
          className={`flex-grow md:max-w-md ${isMobileDetailView ? 'hidden md:block' : ''}`}
        >
          <CardHeader>
          ...
          <CardContent>
            ...
            <ScrollArea className="h-[400px] md:h-[calc(100vh-300px)]">
              {filteredChats.map((chat) => (
                <div
                  key={chat.id}
                  className={`flex items-center space-x-4 p-2 rounded-lg cursor-pointer hover:bg-muted ${
        ...
        <div
          className={`flex-grow space-y-4 sm:space-y-6 ${isMobileDetailView ? '' : 'hidden md:block'}`}
        >
          <Card>
            ...
            <CardContent>
              <Tabs defaultValue="chat">
                ...
                <TabsContent value="chat" className="space-y-4">
                  <div className="h-[300px] md:h-[400px] border rounded-lg p-4 overflow-y-auto">
